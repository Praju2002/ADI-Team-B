...
plotly_chart_with_labels(df_filtered, x_col='date_YY', y_col='safely_managed_pct', chart_label="Population with Safely Managed Sanitation", flag = 0)

# KPI 3
df = w_service_df
df["Efficiency"] = df.apply(
    lambda row: ((row["w_supplied"] - row["total_consumption"]) / row["w_supplied"]) * 100
    if row["w_supplied"] != 0 else 0,
    axis=1
).round(2)
df_filtered = df[df[filter_col].isin(selected_values)]
plotly_chart_with_labels(df_filtered, x_col='date_MMYY', y_col='Efficiency', chart_label="Non-Revenue Water (NRW)", flag = 0)
