...
if not billing_filtered.empty and 'billed' in billing_filtered.columns and 'paid' in billing_filtered.columns:
    ...
    if 'date' in df_nrw.columns:
        df_nrw['date'] = pd.to_datetime(df_nrw['date'], errors='coerce')
    else:
        df_nrw['date'] = pd.NaT
    
    df_nrw['country'] = df_nrw.get('country', 'Unknown')
    df_nrw['year_month'] = df_nrw['date'].dt.to_period('M').astype(str)
    
    # Aggregate by country and month
    nrw_agg = df_nrw.groupby(['country', 'year_month']).agg({
        'billed': 'sum',
        'paid': 'sum'
    }).reset_index()
    
    # Calculate NRW Amount and Rate
    ...
    nrw_agg = nrw_agg[nrw_agg['billed'].notna()]

    # If billed sums are zero or no meaningful data, try volumetric computation
    total_billed = nrw_agg['billed'].sum() if not nrw_agg.empty else 0
    if (nrw_agg.empty) or (total_billed == 0):
        ...
        if best.get('mode') == 'volumetric':
            # Render volumetric NRW view
            bal = best['national']
            zone_col = best.get('zone_col', 'zone')
            col1, col2, col3 = st.columns(3)
            avg_nrw = bal['nrw_pct_of_input'].mean()
            total_nrw_m3 = bal['nrw_m3'].sum()
            total_input = bal['system_input_m3'].sum()
            ...
            with col2:
                st.metric("Total NRW Volume", f"{total_nrw_m3:,.0f} m3", help="Total volumetric NRW")

            with col3:
                st.metric("Total System Input", f"{total_input:,.0f} m3", help="Total production / system input")

            # Plot
            fig = make_subplots(specs=[[{"secondary_y": True}]])
            fig.add_trace(
                go.Bar(x=bal['year_month'], y=bal['nrw_m3'], name='NRW (m3)', marker_color='lightgray', opacity=0.6),
                secondary_y=True
            )
            fig.add_trace(
                go.Scatter(x=bal['year_month'], y=bal['nrw_pct_of_input'], name='NRW % of Input', mode='lines+markers', line=dict(color=OKABE_ITO[4], width=2)),
                secondary_y=False
            )
            fig.update_xaxes(title_text='Period')
            fig.update_yaxes(title_text='NRW Rate (%)', secondary_y=False)
            fig.update_yaxes(title_text='NRW Volume (m3)', secondary_y=True)
            try:
                set_smart_yaxis(fig, primary=bal['nrw_pct_of_input'], secondary=bal['nrw_m3'])
            except Exception:
                pass
            try:
                ...
                pass
            st.plotly_chart(fig, use_container_width=True, key='nrw_volumetric')
        else:
            st.warning("Billing data present but not suitable for NRW and volumetric data not available.")
        ...
        # Charts
        # Wide line chart with overlay bar for NRW amount
        fig = make_subplots(specs=[[{"secondary_y": True}]])
        
        # Add overlay bar for NRW amount (light gray, thin)
        fig.add_trace(
            go.Bar(
                x=nrw_agg['year_month'],
                y=nrw_agg['nrw_amount'],
                name='NRW Amount',
        ...
        # Add line for NRW Rate (blue)
        for country in nrw_agg['country'].unique():
            country_data = nrw_agg[nrw_agg['country'] == country]
            fig.add_trace(
                go.Scatter(
                    x=country_data['year_month'],
                    y=country_data['nrw_rate'],
                    name=f'{country} - NRW Rate',
                    mode='lines+markers',
                    line=dict(color=OKABE_ITO[4], width=2),
                    marker=dict(size=6, color=OKABE_ITO[4])
                ),
                secondary_y=False
            )
        ...
        fig.update_xaxes(title_text="Period")
        fig.update_yaxes(title_text="NRW Rate (%)", secondary_y=False)
        fig.update_yaxes(title_text="NRW Amount", secondary_y=True)
        
        fig.update_layout(
            title='NRW Rate Over Time with NRW Amount Overlay',
        ...
        try:
            set_smart_yaxis(fig, primary=nrw_agg['nrw_rate'], secondary=nrw_agg['nrw_amount'])
        except Exception:
            pass
        ...
        st.plotly_chart(fig, use_container_width=True, key="nrw_trend")
        
        with st.expander("Understanding NRW Rate"):
            st.markdown("""
            **Non-Revenue Water (NRW) Rate** measures the percentage of water that is billed but not paid for.
    ...
    # or does not contain billed/paid columns. If billing-based NRW
    # has already been computed above, do not show volumetric view.
    if billing_filtered.empty or ('billed' not in billing_filtered.columns) or ('paid' not in billing_filtered.columns):
        prod_df = load_table('production')
        ws_df = load_table('w_service')
        best = compute_best_nrw(billing_filtered, prod_df, ws_df, selected_country)
        if best.get('mode') == 'volumetric':
            ...
            # For Uganda we may prefer only the plot; metrics handled elsewhere
            if str(selected_country).strip().lower() != 'uganda':
                col1, col2, col3 = st.columns(3)
            ...
            try:
                ...
                pass
            st.plotly_chart(fig, use_container_width=True, key='nrw_volumetric_fallback')
...
# ===========================
st.markdown("---")
st.markdown("### Water Operations Summary")

summary_data = []

if 'avg_nrw' in summary_metrics:
    avg = summary_metrics['avg_nrw']
    summary_data.append({
...
if 'avg_etp' in summary_metrics:
    avg = summary_metrics['avg_etp']
    summary_data.append({
...
if 'avg_ctp' in summary_metrics:
    avg = summary_metrics['avg_ctp']
    summary_data.append({
...
if 'avg_puw' in summary_metrics:
    avg = summary_metrics['avg_puw']
    summary_data.append({
...
st.markdown("---")
st.markdown("""
### Water Operations Dashboard Summary

This dashboard tracks **Water Operational Performance** through key indicators:

1. **NRW Rate**: Non-Revenue Water / billing collection efficiency
2. **ETP Rate**: E.Coli water quality tests
3. **CTP Rate**: Chlorine water quality tests
4. **Water Access**: Population distribution by service level over time
5. **PUW Rate**: Population without water connection
